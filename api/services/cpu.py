...
class CPUPlayer:
    ...
    async def make_move(self, game_id: UUID, state: GameState) -> None:
        ...
        # Hard (Human-like RL) handles pick-chains via legal_actions.
    ...
    async def _make_move_hard(self, game_id: UUID, game: WhotGame, player: Player, state: GameState) -> None:
        """Human-like RL policy; fall back to normal heuristic if unavailable."""
        try:
            from rl.infer import get_rl_opponent

            checkpoint = settings.rl_checkpoint_path or None
            opponent = get_rl_opponent(checkpoint)
            if opponent is None:
                logger.warning("RL opponent unavailable; falling back to normal")
                await self._fallback_heuristic(game_id, game, player, state)
                return

            action = opponent.choose(state, self.player_id, random.Random())
            if action.kind == "PICK":
                await self.pick_card(game_id, game)
                return

            if action.requested_shape:
                self._last_whot_shape = action.requested_shape
            await self.play_card(game_id, game, player, list(action.cards), state)
        except Exception as e:
            logger.error("RL move failed for player %s: %s", self.player_id, e)
            await self._fallback_heuristic(game_id, game, player, state)

    async def _fallback_heuristic(
        self, game_id: UUID, game: WhotGame, player: Player, state: GameState
    ) -> None:
        if state.pick_chain_count > 0:
            if self.can_defend_pick_chain(player, state):
